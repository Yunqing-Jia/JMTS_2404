...
class DijkstraGUI:
    ...
    def create_widgets(self):
        ...
        # Commented out image loading to avoid file dependency issues
    ...
    def update_node_name(self, node_id, new_name):
        ...
        if new_name.strip():
            ...
            self.update_combos()
            self.draw_network()  # Redraw network to show updated names
    ...
    def create_matrix_widgets(self):
        ...
        origin_label = ttk.Label(origin_dest_frame, text="origin↓",
                                 font=('Arial', 8, 'bold'), justify='center')
        origin_label.pack()
        # Column headers - show names instead of IDs
        for j, node_id in enumerate(node_ids):
            header_label = ttk.Label(self.matrix_frame, text=self.nodes_dict[node_id]['name'],
                                     font=('Arial', 9, 'bold'))
            header_label.grid(row=0, column=j + 1, padx=2, pady=2, sticky='nsew')
        # Row headers and matrix entries - show names instead of IDs
        self.matrix_entries = {}
        for i, from_node in enumerate(node_ids):
            # Row header - show name instead of ID
            row_label = ttk.Label(self.matrix_frame, text=self.nodes_dict[from_node]['name'],
                                  font=('Arial', 9, 'bold'))
    ...
    def update_combos(self):
        """Update start and end combo boxes with names"""
    ...
    def draw_network(self):
        ...
        nx.draw_networkx_nodes(self.graph, pos, ax=self.ax,
                               ...
                               edgecolors='navy',
                               linewidths=2)
        # Draw node labels using names instead of IDs
        node_labels = {node_id: self.nodes_dict[node_id]['name'] for node_id in node_ids}
    ...
    def calculate_shortest_path(self):
        ...
        start_id = self.get_node_id_by_name(start_name)
        end_id = self.get_node_id_by_name(end_name)
        ...
        try:
            ...
            # Show result using names instead of IDs
